...
if($clear){
	foreach ($_COOKIE['view'] as $key => $value)
	{	
		setcookie("view[".$key."]", "" ,1);
	}
	header("Refresh:0; url=index.php");
}	
?>
...
<div class='content'>
	...
	<?php 
		$i=0;
		$q_cp=mysqli_query($conn, 'Select * from cpu where cpu.quanity>50');	
		$q_gp=mysqli_query($conn, 'Select * from gpu where gpu.quanity>42');
		If($q_cp || $q_gp){
			a:
	...
	<a class='clear' href='index.php?clear=1'>Wyczyść</a>	
	</div>
	<br>
