...
<div class='content'>
	<div class="line">
		<h1><a href="cpu.php">Procesory</a></h1>
		<h2><a href="cpu.php?cpu=Intel" > Intel</a></h2> <h2>||</h2>
		<h2><a href="cpu.php?cpu=AMD" >AMD</a></h2>
	</div>
	<br>
	<?php 
		$quest=mysqli_query($conn, 'Select * from cpu '.$kind.' Order by quanity DESC');
		If($quest){
			while($row=mysqli_fetch_assoc($quest)){
				echo("<div class='block' onclick='getid(this.id)' id='cpu_".$row['cpu_id']."'>");
				...
				echo("</div>");
				echo("<p class='price'>".$row['price']." zł</p><br>");
				echo("</div>");
			}
		}
	?>
